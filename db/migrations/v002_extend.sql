...
WHERE s.iupac_short = 'α-D-Glcp-(1→4)-D-Glcp' AND e.nmr_type = '1H'
  AND NOT EXISTS (SELECT 1 FROM nmr_shifts_1d sh
                  WHERE sh.experiment_id = e.experiment_id
                    AND sh.nucleus = '1H' AND sh.shift_ppm = 5.410);
...
WHERE s.iupac_short = 'α-D-Glcp-(1→4)-D-Glcp' AND l.doi = '10.1021/acs.joc.0c00000'
  AND NOT EXISTS (SELECT 1 FROM nmr_experiments e
                  WHERE e.sugar_id = s.sugar_id AND e.source_id = l.source_id
                    AND e.nmr_type = '2D' AND e.solvent = 'D2O');
...
WHERE s.iupac_short = 'α-D-Glcp-(1→4)-D-Glcp' AND e.nmr_type = '2D'
  AND NOT EXISTS (SELECT 1 FROM nmr_correlations_2d c
                  WHERE c.experiment_id = e.experiment_id AND c.experiment_2d = 'HSQC'
                    AND c.proton_shift_ppm = 5.410 AND c.hetero_shift_ppm = 100.600);
...
WHERE s.iupac_short = 'α-D-Glcp-(1→4)-D-Glcp' AND e.nmr_type = '2D'
  AND NOT EXISTS (SELECT 1 FROM nmr_correlations_2d c
                  WHERE c.experiment_id = e.experiment_id AND c.experiment_2d = 'HMBC'
                    AND c.proton_shift_ppm = 5.410 AND c.hetero_shift_ppm = 78.300);
...
FROM sugars s, literature l
WHERE s.iupac_short = 'α-D-Glcp-(1→4)-D-Glcp' AND l.doi = '10.1021/acs.joc.0c00000'
  AND NOT EXISTS (SELECT 1 FROM physicochemical p
                  WHERE p.sugar_id = s.sugar_id AND p.source_id = l.source_id);
...
FROM sugars s WHERE s.iupac_short = 'β-D-Fruf-(2→1)-[Inulin]'
  AND NOT EXISTS (SELECT 1 FROM polysaccharide_props pp WHERE pp.sugar_id = s.sugar_id);
...
WHERE s.iupac_short = 'β-D-Fruf-(2→1)-[Inulin]' AND e.nmr_type = '13C'
  AND NOT EXISTS (SELECT 1 FROM nmr_shifts_1d sh
                  WHERE sh.experiment_id = e.experiment_id
                    AND sh.nucleus = '13C' AND sh.shift_ppm = 104.200);
...
WHERE s.iupac_short = 'β-D-Fruf-(2→1)-[Inulin]' AND e.nmr_type = '13C'
  AND NOT EXISTS (SELECT 1 FROM nmr_shifts_1d sh
                  WHERE sh.experiment_id = e.experiment_id
                    AND sh.nucleus = '13C' AND sh.shift_ppm = 78.000);
